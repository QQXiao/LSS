function out = single_event_model_ls2(featdir,condfile,write_std_image,use_raw, fsldir)
   ...
   %trial" regressor into two or more (here I had 4)
    condlist=load(condfile);
   for i=1:nparam
       keep_loop=ones(size(condlist));
       keep_loop(i)=0;
       conlist_loop=condlist(keep_loop==1);
       X_omit=X_single(:, keep_loop==1);
       des_loop=[X_single(:,i), sum(X_omit(:,conlist_loop==1),2), sum(X_omit(:,conlist_loop==2),2), sum(X_omit(:,conlist_loop==3),2), nuisance];
       %des_loop=F*des_loop;
       des_loop=des_loop-kron(ones(length(des_loop),1),mean(des_loop));
       beta_maker_loop=pinv(des_loop);
       beta_maker(i,:)=beta_maker_loop(1,:);
